  ...
  return (
    <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur md:px-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-xs font-medium uppercase text-slate-500">{user.organizationName ?? "Organização"}</p>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden text-right sm:block">
            ...
            <p className="text-xs text-slate-500">{user.departmentName ?? user.role}</p>
          </div>
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-sky-700 text-sm font-semibold text-white">
            {getInitials(user.name ?? user.email)}
          </div>
          <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">Sem login</span>
        </div>
      </div>
    </header>
  );
